...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable, IDamageable
	{
		...
		private Race m_Race;
		private int m_SpellHue;
        #endregion



		[CommandProperty(AccessLevel.GameMaster)]
		public int SpellHue { get => m_SpellHue; set => m_SpellHue = value; }
		
        private static readonly TimeSpan WarmodeSpamCatch = TimeSpan.FromSeconds((Core.SE ? 0.1 : 0.1));
		private static readonly TimeSpan WarmodeSpamDelay = TimeSpan.FromSeconds((Core.SE ? 0.1 : 0.1));

		private const int WarmodeCatchCount = 40;
		// Allow 40 warmode changes in 0.5 seconds, any more will be delay for two seconds
		...
		*/
		...
		private static int m_WalkFoot = 300;
		...
		public virtual bool CanUseStuckMenu()
		{
			...
			else
			{
				...
				{
					if ((DateTime.UtcNow - m_StuckMenuUses[i]) > TimeSpan.FromDays(0.0)) // default 1.0
					{
						return true;
					}
				}
		...
		public virtual bool RetainPackLocsOnDeath { get { return Core.LBR; } }
		...
		private static int m_ActionDelay = 100; // changed from 500
		...
		public void UsedStuckMenu()
		{
			if (m_StuckMenuUses == null)
			{
				m_StuckMenuUses = new DateTime[4];
			}
