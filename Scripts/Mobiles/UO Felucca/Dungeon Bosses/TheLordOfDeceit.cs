...
{
    [CorpseName("a Lord's corpse")]
    public class TheLordOfDeceit : BaseCreature
    {
        [Constructable]
        public TheLordOfDeceit()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Lord of Deceit";
            NameHue = 33;
            Body = 78;
            Hue = 2740;
            BaseSoundID = 412;
        ...
        public TheLordOfDeceit(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new DeceitBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new DeceitBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new DeceitCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new DeceitLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new DeceitTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new DeceitSpellbook());
        }
