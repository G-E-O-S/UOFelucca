...
{
    [CorpseName("a King's corpse")]
    public class TheKingOfHythloth : BaseCreature
    {
        [Constructable]
        public TheKingOfHythloth()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the King of Hythloth";
            NameHue = 33;
            Body = 40;
            Hue = 2759;
            BaseSoundID = 357;
        ...
        public TheKingOfHythloth(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new HythlothBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new HythlothBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new HythlothCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new HythlothLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new HythlothTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new HythlothSpellbook());
        }
