...
namespace Server.Mobiles
{
    ...
    public class TheLordOfFire : BaseCreature
    {
        [Constructable]
        public TheLordOfFire()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Lord of Fire";
            NameHue = 33;
            Body = 9;
            Hue = 2076;
            BaseSoundID = 357;
        ...
        public TheLordOfFire(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new FireBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new FireBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new FireCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new FireLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new FireTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new FireSpellbook());
        }
