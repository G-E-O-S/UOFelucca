...
namespace Server.Mobiles
{
    ...
    public class TheKingOfDestard : BaseCreature
    {
        [Constructable]
        public TheKingOfDestard()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the King of Destard";
            NameHue = 33;
        ...
        public TheKingOfDestard(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new DestardBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new DestardBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new DestardCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new DestardLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new DestardTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new DestardSpellbook());
        }
