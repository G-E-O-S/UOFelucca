...
namespace Server.Mobiles
{
    ...
    public class TheLadyOfIce : BaseCreature
    {
        [Constructable]
        public TheLadyOfIce()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Lady of Ice";
            NameHue = 33;
            Body = 252;
            Hue = 2746;
            BaseSoundID = 0x482;
        ...
        public TheLadyOfIce(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new IceCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new IceLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new IceTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new IceSpellbook());
        }
