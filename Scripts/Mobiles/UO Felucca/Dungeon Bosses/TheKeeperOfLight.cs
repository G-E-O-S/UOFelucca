...
{
    [CorpseName("a keeper's corpse")]
    public class TheKeeperOfLight : BaseCreature
    {
        [Constructable]
        public TheKeeperOfLight()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Keeper of Light";
            NameHue = 33;
            Body = 772;
            Hue = 2748;
            BaseSoundID = 0x28B;
        ...
        public TheKeeperOfLight(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            c.DropItem(new PrismOfLightCloth());
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new PrismOfLightBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new PrismOfLightBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new PrismOfLightCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new PrismOfLightLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new PrismOfLightTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new PrismOfLightSpellbook());
        }
