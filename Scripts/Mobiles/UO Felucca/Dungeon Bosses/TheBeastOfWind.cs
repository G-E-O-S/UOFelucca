...
namespace Server.Mobiles
{
    ...
    public class TheBeastOfWind : BaseCreature
    {
        [Constructable]
        public TheBeastOfWind()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Beast of Wind";
            NameHue = 33;
            Body = 75;
            Hue = 2741;
            BaseSoundID = 604;
        ...
        public TheBeastOfWind(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new WindBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new WindBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new WindCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new WindLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new WindTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new WindSpellbook());
        }
