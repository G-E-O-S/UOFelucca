...
namespace Server.Mobiles
{
    ...
    public class TheKingOfDespise : BaseCreature
    {
        [Constructable]
        public TheKingOfDespise()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the King of Despise";
            NameHue = 33;
            Body = 76;
            Hue = 1161;
            BaseSoundID = 609;
        ...
        public TheKingOfDespise(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new DespiseBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new DespiseBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new DespiseCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new DespiseLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new DespiseTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new DespiseSpellbook());
        }
