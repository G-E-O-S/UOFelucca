...
namespace Server.Mobiles
{
    ...
    public class TheHorrorOfKhaldun : BaseCreature
    {
        [Constructable]
        public TheHorrorOfKhaldun()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Horror of Khaldun";
            NameHue = 33;
            Body = 8;
            Hue = 2756;
            BaseSoundID = 352;
        ...
        public TheHorrorOfKhaldun(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new KhaldunBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new KhaldunBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new KhaldunCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new KhaldunLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new KhaldunTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new KhaldunSpellbook());
        }
