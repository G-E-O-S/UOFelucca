...
namespace Server.Mobiles
{
    ...
    public class TheLordOfShame : BaseCreature
    {
        [Constructable]
        public TheLordOfShame()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Lord of Shame";
            NameHue = 33;
            Body = 159;
            Hue = 2758;
            BaseSoundID = 278;
        ...
        public TheLordOfShame(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new IceBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new IceBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new ShameCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new ShameLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new ShameTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new ShameSpellbook());
        }
