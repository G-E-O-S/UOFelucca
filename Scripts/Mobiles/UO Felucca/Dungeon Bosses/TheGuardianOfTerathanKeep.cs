...
namespace Server.Mobiles
{
    ...
    public class TheGuardianOfTerathanKeep : BaseCreature
    {
        [Constructable]
        public TheGuardianOfTerathanKeep()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Guardian of Terathan Keep";
            NameHue = 33;
            Body = 104;
            Hue = 1177;
            BaseSoundID = 0x488;
        ...
        public TheGuardianOfTerathanKeep(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            c.DropItem(new TerathanKeepCloth());
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new TerathanKeepBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new TerathanKeepBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new TerathanKeepCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new TerathanKeepLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new TerathanKeepTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new TerathanKeepSpellbook());
        }
