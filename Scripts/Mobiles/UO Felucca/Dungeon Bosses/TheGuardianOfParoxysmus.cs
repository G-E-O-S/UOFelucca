...
namespace Server.Mobiles
{
    ...
    public class TheGuardianOfParoxysmus : BaseCreature
    {
        [Constructable]
        public TheGuardianOfParoxysmus()
            : base(AIType.AI_Mystic, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            Name = "the Guardian of Paroxysmus";
            NameHue = 33;
            Body = 148;
            Hue = 2720;
            BaseSoundID = 451;
        ...
        public TheGuardianOfParoxysmus(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void OnDeath(Container c)
        {
            ...
            c.DropItem(new PalaceOfParoxysmusCloth());
            ...
            if (Utility.RandomDouble() < 0.30)
                c.DropItem(new PalaceOfParoxysmusBackpackDye());
            if (Utility.RandomDouble() < 0.20)
                c.DropItem(new PalaceOfParoxysmusBeardDye());
            if (Utility.RandomDouble() < 0.10)
                c.DropItem(new PalaceOfParoxysmusCandle());
            if (Utility.RandomDouble() < 0.08)
                c.DropItem(new PalaceOfParoxysmusLantern());
            if (Utility.RandomDouble() < 0.06)
                c.DropItem(new PalaceOfParoxysmusTorch());
            if (Utility.RandomDouble() < 0.04)
                c.DropItem(new PalaceOfParoxysmusSpellbook());
        }
