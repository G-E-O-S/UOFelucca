    ...
    public class Barracoon : BaseChampion
    {
        ...
        public override Type[] SharedList
        {
            get
            {
                return new Type[]
                {
                    typeof(AncientMasteryEarrings),
                    typeof(GrandRangerBracelet),
                    typeof(MasterFinesseRing),
                    typeof(RoyalArtistryNecklace)
                };
            }
        }
        public override Type[] DecorativeList
        {
            get
            {
                return new Type[] { typeof(SwampTile), typeof(EvilSpellbook) };
            }
        }
        ...
        public void SpawnRatmen(Mobile target)
        {
            ...
            if (rats < 16)
            {
                ...
                int newRats = Utility.RandomMinMax(1, 2);
        ...
        public void DoSpecialAbility(Mobile target)
        {
            ...
            if (target.Player && 0.2 >= Utility.RandomDouble()) // 20% chance to polymorph attacker into a ratman
                Polymorph(target);

            if (0.05 >= Utility.RandomDouble()) // 5% chance to more ratmen
                SpawnRatmen(target);

            if (0.01 >= Utility.RandomDouble() && !IsBodyMod) // 1% chance to polymorph into a ratman
                Polymorph(this);
        }
