...
namespace Server.Mobiles
{
    ...
    public class BaseCreature : Mobile, IHonorTarget, IEngravable
    {
        ...
        private int evoPointsToEvolve;
        private int evoPointsToEvolveMin;
        private int evoPointsToEvolveMax;
        private int evoStage;
        private int evoMaxStage;

        private bool isEvoPet;

        private List<int> EvoStageBodyValues;

        public virtual bool AddPointsOnDamage { get { return true; } }
        public virtual bool AddPointsOnMelee { get { return false; } }
        public virtual bool AddPointsOnKill { get { return true; } }

        [CommandProperty(AccessLevel.Administrator)]
        public int EvoPoints
        {
            get { return evoPoints; }
            set
            {
                evoPoints = value;
        ...
        [CommandProperty(AccessLevel.Administrator)]
        public int EvoPointsToEvolve
        {
            get { return evoPointsToEvolve; }
            set
            {
                evoPointsToEvolve = value;
                InvalidateProperties();
            }
        }

        [CommandProperty(AccessLevel.Administrator)]
        public int EvoPointsToEvolveMin
        {
            get { return evoPointsToEvolveMin; }
            set
            {
                evoPointsToEvolveMin = value;
                InvalidateProperties();
            }
        }

        [CommandProperty(AccessLevel.Administrator)]
        public int EvoPointsToEvolveMax
        {
            get { return evoPointsToEvolveMax; }
            set
            {
                evoPointsToEvolveMax = value;
                InvalidateProperties();
            }
        }
        ...
        [CommandProperty(AccessLevel.Administrator)]
        public int EvoMaxStage
        {
            get { return evoMaxStage; }
            set
            {
                evoMaxStage = value;
                InvalidateProperties();
        ...
        public virtual double TeleportChance { get { return 0.00; } } // CHANGED FROM 0.05
        ...
        public override bool CheckPoisonImmunity(Mobile from, Poison poison)
        {
            ...
            // if the pet already has a poison immunity, we add the two values for a new immunity total
            if (p != null && xp != null)
            {
                int getPoisonImmunity = 0;

                if (p.RealLevel < 1)
                {
                    getPoisonImmunity = 0;
                }
                else if (p.RealLevel == 1)
                {
                    getPoisonImmunity = 1;
                }
                else if (p.RealLevel == 2)
                {
                    getPoisonImmunity = 2;
                }
                else if (p.RealLevel == 3)
                {
                    getPoisonImmunity = 3;
                }
                else if (p.RealLevel > 3)
                ...
                int totalImmuneLevel = (getPoisonImmunity + xp.ImmuneLevel);

                return (p != null && totalImmuneLevel >= poison.RealLevel);
            }

            // if it doesn't already have an immunity, we use the poison immunity bonus ONLY
            else if (p == null && xp != null)
            {
                return (xp.ImmuneLevel >= poison.RealLevel);
            }

            // if there is no poison immunity bonus
        ...
        */
        ...
        {            
            ...
            if (BardPacified && (HitsMax - Hits) >= (HitsMax * 0.33)) // default if (BardPacified && (HitsMax - Hits) * 0.001 > Utility.RandomDouble())
                {
                Unpacify();
            }
            ...
            AIType ai, FightMode mode, int iRangePerception, int iRangeFight, double dActiveSpeed, double dPassiveSpeed)
        {
            ...
            // FS:ATS
            Timer.DelayCall(FsAtsSetup);
            // Evo Pets
            IsEvoPet = false;
            EvoStage = 0;
            EvoPointsToEvolve = 50;
            EvoMaxStage = 6;

            // xml sockets
            Timer.DelayCall(Sockets);
        ...
        #region FS:ATS
        private void FsAtsSetup()
        {
            if (!Tamable)
                return;

            #region CUSTOM [FS:ATS]
            m_NextLevel = (m_Level * 1000);
            var type = GetType().Name;
            var alwaysMale = false;
            var alwaysFemale = false;

            foreach (var check in FSATS.AlwaysMale)
            {
                if (check == type)
                {
                    alwaysMale = true;
            ...
            foreach (var check in FSATS.AlwaysFemale)
            {
                if (check == type)
                {
                    alwaysFemale = true;
            ...
            if (alwaysMale)
                Female = false;
            else if (alwaysFemale)
                Female = true;
            else
                Female = Utility.RandomBool();

            MaxLevel = Utility.RandomMinMax(10, 15);
            #endregion
        }
        #endregion
        ...
        private void Sockets()
        {
            ...
            if (this.Map != Map.Internal)  // prevents socketed items being generated on NPC vendors - Blacksmith, Weaponsmith, etc
                XmlSockets.ConfigureRandom(this, 100, 0, 0, 0, 0, 0); // default (this, 2.0, 100, 0.5, 3.0, 15.0, 50.0);

            XmlPoison xmlPoison = (XmlPoison)XmlAttach.FindAttachment(this, typeof(XmlPoison));
            XmlAreaDamage xmlAreaDamage = (XmlAreaDamage)XmlAttach.FindAttachment(this, typeof(XmlAreaDamage));

            if (this.Tamable && xmlPoison == null)
                XmlAttach.AttachTo(this, new XmlPoison(0));
            if (this.Tamable && xmlAreaDamage == null)
                XmlAttach.AttachTo(this, new XmlAreaDamage(0));
        }

        // PetLeveling.CheckLevel(this, bc, toCheck.Count);
        public void AddPoints(Mobile defender, BaseCreature attacker, int count)
        {
            int evoPointsToAdd = (
                Utility.RandomMinMax(
                    5 + (defender.HitsMax) / 10,
                    5 + (defender.HitsMax) / 10
                )
            );

            // if more than one evo pet is attacking the mob, then points earned 
            // are divided by number of pets who have done damage
            if (count > 1)
            {
                evoPointsToAdd /= evoPointsToAdd;
            }

            attacker.EvoPoints += evoPointsToAdd;

            if (attacker.ControlMaster != null)
            {
                attacker.ControlMaster.SendMessage(1153, $"Your pet {attacker.Name} has gained {evoPointsToAdd} evo points.");
            }

            if (attacker.EvoStage < attacker.EvoMaxStage && attacker.EvoPoints >= attacker.EvoPointsToEvolve)
            {
                attacker.Evolve(attacker);

                if (attacker.ControlMaster != null);
                attacker.SendMessage(1161, $"{attacker.Name}'s has reached Evo Stage {attacker.EvoStage}.");
            }
        }

        public virtual void Evolve(BaseCreature attacker)
        {
            EvoPoints -= EvoPointsToEvolve;
            EvoStage++;
            EvoPointsToEvolve = EvoStage * 5000; // 5000;
            this.FixedParticles(0x37B9, 10, 15, 5012, EffectLayer.Waist);
            PlaySound(665);
            Emote($"{Name} has evolved!");
            attacker.SendMessage(1161, $"{attacker.Name}'s has reached Evo Stage {attacker.EvoStage}.");
        ...
        // This is the default code, but can be overriden to change values for individual evo pet types
        public virtual void RaiseSkills()
        {
        ...
        // This is the default code, but can be overriden to change values for individual evo pet types
        public virtual void RaiseStats()
        {
            this.RawStr += 15;
            this.RawDex += 15;
            this.RawInt += 15;
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            #region CUSTOM [FS:ATS]
            writer.Write(0x0C0FFEE0); // preamble

            writer.Write(3); // subversion

            // subversion 3
            writer.Write((double)bonusGold);

            // subversion 2
            writer.Write((bool)isEvoPet);

            // subversion 1 - Evo System - custom, NOT part of of FS:ATS
            writer.Write((int)evoPoints);
            writer.Write((int)evoPointsToEvolve);
            writer.Write((int)evoPointsToEvolveMin);
            writer.Write((int)evoPointsToEvolveMax);
            writer.Write((int)evoStage);
            writer.Write((int)evoMaxStage);
            ...
            if (reader.PeekInt() == 0x0C0FFEE0 && reader.ReadInt() == 0x0C0FFEE0)
            {
                ...
                switch (subversion)
                {
                    ...
                    case 1:
                        {
                            // Evo System
                            evoPoints = reader.ReadInt();
                            evoPointsToEvolve = reader.ReadInt();
                            evoPointsToEvolveMin = reader.ReadInt();
                            evoPointsToEvolveMax = reader.ReadInt();
                            evoStage = reader.ReadInt();
                            evoMaxStage = reader.ReadInt();
        ...
        */
        ...
        {
            #region CUSTOM [FS:ATS]
            if (from.Alive && Alive && Controlled && ControlMaster == from && !Summoned && FSATS.EnablePetLeveling)
            {
                var type = GetType().Name;
                var nolevel = false;

                foreach (var check in FSATS.NoLevelCreatures)
                {
                    if (check == type)
                    {
                        nolevel = true;
            ...
            XmlSockets socket = (XmlSockets)XmlAttach.FindAttachment(this, typeof(XmlSockets));

            if (socket != null && Controlled && from == ControlMaster)
            {
                list.Add(new SocketsEntry(from, this));
            }
        }

        private class SocketsEntry : ContextMenuEntry
        {
            private readonly BaseCreature m_Mobile;

            public SocketsEntry(Mobile from, BaseCreature creature) : base(1049600, 6)
            {
                m_Mobile = creature;
            }
            ...
            public override void OnClick()
            {
                ...
                XmlSockets socket = (XmlSockets)XmlAttach.FindAttachment(m_Mobile, typeof(XmlSockets));

                if (socket != null)
                {
                    Owner.From.CloseGump(typeof(XmlSockets.SocketsGump));
                    Owner.From.SendGump(new XmlSockets.SocketsGump(Owner.From, socket));
                }
            }
        }
        ...
        public void PackNecroReg()
        {
            ...
            //PackItem(Loot.RandomNecromancyReagent()); //commenting this will disable necromancy reagents getting into loot tables
        }
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            if (Tamable && FSATS.EnablePetBreeding)
            {
                ...
                {
                    if (Female)
                        number = 1015328;  //list.Add(1060658, "Gender\tFemale");
                    else
                        number = 1015327; //list.Add(1060658, "Gender\tMale");
            ...
            if (socket != null && Controlled && from == ControlMaster)
                PrivateOverheadMessage(MessageType.Regular, 0x3B2, true, $"Sockets {socket.NFree} of {socket.NSockets} available", from.NetState);
            ...
            if (Tamable)
            {
                ...
                {
                    PrivateOverheadMessage(MessageType.Regular, 1161, 1060658, String.Format("Level\t{0} / {1}", Level.ToString(), MaxLevel.ToString()), from.NetState);
                }
            }
        ...
        public override bool OnBeforeDeath()
        {
            ...
            {
                var toCheck = new HashSet<BaseCreature>();
                var rights = DamageEntries;

                foreach (var entry in rights)
                {
                    if (entry.Damager is BaseCreature bc && bc != this)
                    {
        ...
        public override void OnDeath(Container c)
        {
            ...
            if (Utility.RandomDouble() < 0.001)
                c.DropItem(new ScavengerIngot());
            if (this.LastKiller is PlayerMobile pm)
            {
                var honorPoints = Fame / 100; 

                pm.Virtues.Honor += honorPoints;
            ...
            else
            {
                ...
                if (!Summoned && !m_NoKillAwards)
                {
                    ...
                    for (int i = 0; i < list.Count; ++i)
                    {
                        ...
                        // xml socket gold bonus
                        if (this.LastKiller != null && this.LastKiller is BaseCreature && this.LastKiller.Alive && ((BaseCreature)LastKiller).BonusGold > 0)
                        {
                            //Console.WriteLine($"base creature found\n");

                            BonusGoldLoot(((BaseCreature)LastKiller), c, ((BaseCreature)LastKiller).BonusGold);
                        }
        ...
        #region CUSTOM
        public void BonusGoldLoot(Mobile killer, Container c, double goldBonus)
        {
            //Console.WriteLine($"Gold bonus {goldBonus}\n");

            if (c != null)
            {
                int totalGold = 0;
                int bonusToGive = 0;
                double GoldBonus = goldBonus;
                ...
                if (c.Items.Count > 0)
                {
                    ...
                    if (totalGold > 0 && GoldBonus > 0)
                    {
                        try
                        {
                            bonusToGive = Convert.ToInt32(totalGold * GoldBonus);

                            //Console.WriteLine($"bonus to give {bonusToGive}\n");

                            BankCheck check = new BankCheck(bonusToGive);
                            c.DropItem(check);
                        }
                        catch
                        { }
        ...
        */
        ...
        private const double ChanceToRummage = 0.05; // default 0.5; // 50%
        ...
        public virtual void OnThink()
        {
            ...
            if (this.Alive && this.Controlled && this.ControlMaster != null && Combatant != null)
            {
                XmlAreaDamage areaDmg = (XmlAreaDamage)XmlAttach.FindAttachment(this, typeof(XmlAreaDamage));

                if (areaDmg != null && areaDmg.ChanceToTrigger > 0 && areaDmg.ChanceToTrigger >= Utility.RandomMinMax(1, 100))
                {
                    Point3D p = this.Location;

                    var targets = XmlAreaDamage.AcquireIndirectTargets(this.ControlMaster, p, 2).ToList();
                    ...
                    foreach (var dam in targets)
                    {
                        ...
                        if (m is BaseCreature)
                        {
                            ...
                            if (bc.Controlled && this.Controlled && bc.ControlMaster == this.ControlMaster)
                                continue;
                        }
                        if (m is PlayerMobile)
                        {
                            ...
                            if (this.Controlled && this.ControlMaster == pm)
                                continue;
                        ...
                        damage = Utility.Random(6, 15); // default 27, 22
                        //damage /= count;
                                                
                        Effects.SendBoltEffect(id, true, 0, false);

                        this.DoHarmful(id);
                        ChainLightningSpell spell = new ChainLightningSpell(this, null);
                        SpellHelper.Damage(spell, id, damage, 0, 0, 0, 0, 100);
                    }
    ...
    {
        private static readonly TimeSpan InternalDelay = TimeSpan.FromMinutes(50.0); // default 5.0
        ...
        {
            m_NextHourlyCheck = DateTime.UtcNow + TimeSpan.FromHours(10.0); // default 1.0
