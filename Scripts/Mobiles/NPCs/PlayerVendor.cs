    ...
    public class PlayerVendor : Mobile
    {
        ...
        public int ChargePerDay
        {
            get
            {
                if (BaseHouse.NewVendorSystem)
                {
                    int charge = ChargePerRealWorldDay / 20; // changed from 12
                    return Math.Min(charge, 10000);  // Cap the charge at 10,000
                }
                else
                {
                    long total = 0;
                    ...
                    int charge = (int)(20 + (total / 500)); // changed from 20 + (total
                    return Math.Min(charge, 10000);  // Cap the charge at 10,000
                }
            }
        }
        ...
        public int ChargePerRealWorldDay
        {
            get
            {
                if (BaseHouse.NewVendorSystem)
                {
                    ...
                    int perDay = (int)(60 + (total / 500) * 1); // changed from x3

                    MerchantsTrinket trinket = FindItemOnLayer(Layer.Earrings) as MerchantsTrinket;

                    if (trinket != null)
                    {
                        perDay -= (int)((double)perDay * ((double)trinket.Bonus / 100));
                    }

                    return Math.Min(perDay, 10000);  // Cap the charge at 10,000
                }
                else
                {
                    return Math.Min(ChargePerDay * 6, 10000); // default 12, capped at 10,000
                }
            }
        }    
