...
namespace Server.SkillHandlers
{
    public class AnimalLore
    {
        ...
        private class InternalTarget : Target
        {
            private static void SendGump(Mobile from, BaseCreature c)
            {
                ...
                else
                {
                    ...
                    if(c.Controlled && c.ControlMaster == from)
                    {
                        ...
                        from.SendGump(new PetLevelGump(c));
                    }
                }
            }
            ...
            {
                if (from.CheckTargetSkill(SkillName.AnimalLore, c, min, 100.0)) // changed from 120.0
                    SendGump(from, c);
                else
            ...
            {                
                ...
                else if (targeted is BaseCreature)
                {
                    ...
                    if (!c.IsDeadPet)
                    {
                        if (c.Body.IsAnimal || c.Body.IsMonster || c.Body.IsSea || c.Body.IsHuman || c.Body.IsEquipment || c.Body.IsGargoyle)
                        {
    ...
    public class AnimalLoreGump : Gump
    {
            ...
            : base(250, 50)
        {
            ...
            #region Mondain's Legacy
            if (c is CuSidhe || c is Werewolf || c is DaemonicWerewolf || c is EvoSteed1 || c is EvoSteed2 || c is EvoSteed3 || c is EvoBeast1 || c is EvoBeast2 || c is EvoBeast3)
                {
