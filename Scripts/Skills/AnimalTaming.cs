	...
	public class AnimalTaming
	{
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(Mobile m)
				: base(Core.AOS ? 6 : 4, false, TargetFlags.None)
			{
				...
				if (targeted is Mobile)
				{
					if (targeted is BaseCreature)
					{
						...
						else if (DarkWolfFamiliar.CheckMastery(from, creature) || from.Skills[SkillName.AnimalTaming].Value >= creature.CurrentTameSkill)
						{
							...
							else if (creature.CanAngerOnTame && 0.05 >= Utility.RandomDouble()) // default 0.95
							{
			...
			private class InternalTimer : Timer
			{
				...
				protected override void OnTick()
				{
					...
					if (!m_Tamer.InRange(m_Creature, Core.AOS ? 8 : 8))
					{
				...
				private bool CanPath()
				{
					...
					if (m_Creature.InRange(new Point3D(p), 8))
					{
						return true;
					}
