    ...
    public class StuckMenu : Gump
    {
        ...
        private void Teleport(StuckMenuEntry entry)
        {
            if (m_MarkUse)
            {
                ...
                new TeleportTimer(m_Mobile, entry, TimeSpan.FromSeconds(10.0 + (Utility.RandomDouble() * 55.0))).Start(); // default * 110.0
            ...
            protected override void OnTick()
            {
                ...
                else
                {
                    ...
                    if (m_Mobile.Map == Map.Trammel || SpellHelper.IsEodon(m_Mobile.Map, m_Mobile.Location))
                        destMap = Map.Felucca; // default trammel
                    else if (m_Mobile.Map == Map.Felucca)
                        destMap = Map.Felucca;
                    else if (m_Mobile.Map == Map.TerMur && !SpellHelper.IsEodon(m_Mobile.Map, m_Mobile.Location))
                        destMap = Map.Felucca; // default termur
                    else if (m_Mobile.Map == Map.Internal)
                        destMap = Map.Felucca; // default m_Mobile.LogoutMap == Map.Felucca ? Map.Felucca : Map.Trammel;
                    else
                        destMap = Map.Felucca; // default m_Mobile.Murderer ? Map.Felucca : Map.Trammel;
