...
namespace Server.Engines.VeteranRewards
{
    public class DaviesLockerAddon : BaseAddon, ISecurable
    {
        ...
        public void TryAddEntry(Item item, Mobile from)
        {
            ...
            else
            {
                ...
                {                    
                    ...
                    m_Entries.Sort((e1, e2) => CompareEntriesByCoordinates(e1, e2));
        ...
        private static int CompareEntriesByCoordinates(DaviesLockerEntry entry1, DaviesLockerEntry entry2)
        {
            int result = entry1.Location.X.CompareTo(entry2.Location.X);

            if (result == 0)
            {
                result = entry1.Location.Y.CompareTo(entry2.Location.Y);
            }
