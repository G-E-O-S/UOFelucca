...
namespace Server.Gumps
{
    public class ClientGump : Gump
    {
            ...
            : base(30, 20)
        {
            ...
            this.AddHtml(10, 10, 380, 20, this.Color(this.Center("UO Felucca"), LabelColor32), false, false);
            ...
            this.AddHtml(70, 36 + (line++ * 20), 200, 20, this.Color("Whisper Player", LabelColor32), false, false);            
            ...
            if (m != null)
            {
                this.AddHtml(14, 36 + (line * 20), 200, 20, this.Color("Player:", LabelColor32), false, false);
                this.AddHtml(70, 36 + (line++ * 20), 200, 20, this.Color(m.Name, LabelColor32), false, false);
                ...
                this.AddHtml(70, 36 + (line++ * 20), 200, 20, this.Color(String.Format("[{1}]", m.Location, m.Map), LabelColor32), false, false); // "{0} [{1}]"
                ...
                AddImage(240, 30, 1620);

                if (from.AccessLevel >= AccessLevel.GameMaster)
                {
                    // Add "Properties" button for GMs and above
                ...
                if (from != m)
                {
                    if (from.AccessLevel >= AccessLevel.GameMaster)
                    {
                        // Add "Go to them" and "Bring them here" buttons for GMs and above
                ...
                if (from.AccessLevel >= AccessLevel.GameMaster)
                {
                    // Add "Move to target" button for GMs and above
