        ...
        private class AddAugmentationToSocket : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is IXmlSocketAugmentation)
                {
                    ...
                    {
                        XmlSockets socket = (XmlSockets)XmlAttach.FindAttachment(m_parent, typeof(XmlSockets));

                        if (socket.SocketOccupants.Count > 0)
                        {
                            for (int i = 0; i < socket.SocketOccupants.Count; i++)
                            {
                                SocketOccupant s = (SocketOccupant)socket.SocketOccupants[i];

                                if (s != null)
                                {
                                    Type socketType = s.OccupantType;
                                    Type targetType = a.GetType();

                                    if (targetType == socketType)
                                    {
                                        from.SendMessage("That pet already has that gem type!");
                                        return;
                                    }
                                }
                            }
