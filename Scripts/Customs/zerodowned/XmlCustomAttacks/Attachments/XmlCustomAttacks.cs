...
namespace Server.Engines.XmlSpawner2
{
    public class XmlCustomAttacks : XmlAttachment
    {
        ...
        public static new void Initialize()
        {
            ...
            AddSpecialAttack("Critical Hit", "Deals triple the max damage of the weapon",        // attack name, and description
                SpecialAttacks.CriticalHit, 0x5201, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                5, 5, 0, 0,                                                                    // mana, stam, hits, karma usage
                90, 70, 25,                                                                        // str, dex, int requirements
                new SkillName[] { SkillName.Swords },                                          //  skill requirement list
                new int[] { 100 },                                                               // minimum skill levels
                new Type[] { typeof(RewardScroll) },                                              // reagent list
                new int[] { 1 }                                                                 // reagent quantities
            );

            AddSpecialAttack("Critical Hit", "Deals triple the max damage of the weapon",        // attack name, and description
                SpecialAttacks.CriticalHit2, 0x5201, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                5, 5, 0, 0,                                                                    // mana, stam, hits, karma usage
                90, 70, 25,                                                                        // str, dex, int requirements
                new SkillName[] { SkillName.Fencing },                                          //  skill requirement list
                new int[] { 100 },                                                               // minimum skill levels
                new Type[] { typeof(RewardScroll) },                                              // reagent list
                new int[] { 1 }                                                                 // reagent quantities
            );

            AddSpecialAttack("Silence Target", "Stuns target for 10 seconds",                     // attack name, and description
                SpecialAttacks.SilenceTarget, 0x5007, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                5, 5, 0, 0,                                                                    // mana, stam, hits, karma usage
                90, 70, 25,                                                                        // str, dex, int requirements
                new SkillName[] { SkillName.Peacemaking },                                          //  skill requirement list
                new int[] { 100 },                                                               // minimum skill levels
                new Type[] { typeof(RewardScroll) },                                              // reagent list
                new int[] { 1 }                                                                 // reagent quantities
            );

            AddSpecialAttack("Lightning Strike", "Hits lightning effect",                     // attack name, and description
                SpecialAttacks.LightningStrike, 0x5008, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                5, 5, 0, 0,                                                                    // mana, stam, hits, karma usage
                90, 70, 25,                                                                        // str, dex, int requirements
                new SkillName[] { SkillName.Macing },                                          //  skill requirement list
                new int[] { 100 },                                                               // minimum skill levels
                new Type[] { typeof(RewardScroll) },                                              // reagent list
                new int[] { 1 }                                                                 // reagent quantities
            );

            AddSpecialAttack("Blood Drink", "Restores Health",                                // attack name, and description
                SpecialAttacks.BloodDrink, 0x500C, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                5, 5, 0, 0,                                                                    // mana, stam, hits, karma usage
                90, 70, 25,                                                                        // str, dex, int requirements
                new SkillName[] { SkillName.Lumberjacking },                                          //  skill requirement list
                new int[] { 100 },                                                               // minimum skill levels
                new Type[] { typeof(RewardScroll) },                                              // reagent list
                new int[] { 1 }                                                                 // reagent quantities
            );

            AddSpecialAttack("Plague Touch", "Applys Poison",                                  // attack name, and description
                SpecialAttacks.PlagueTouch, 0x5002, TimeSpan.FromSeconds(3.5),                   // attack id, id of gump icon, and chaining time
                5, 5, 0, 0,                                                                    // mana, stam, hits, karma usage
                90, 70, 25,                                                                        // str, dex, int requirements
                new SkillName[] { SkillName.Fencing },                                          //  skill requirement list
                new int[] { 100 },                                                               // minimum skill levels
        ...
        public void DoSpecialAttack(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, SpecialAttack special)
        {
            ...
            attacker.SendMessage(1161, "You strike with {0}!", special.Name);

            //int[] bloodHues = { 0x122A, 0x122D, 0x122F };
            //int randomHue = bloodHues[Utility.Random(bloodHues.Length)];

            //Blood blood = new Blood(randomHue);
            ...
            //if (map != null)
            //{
            //    for (int x = defender.X - radius; x <= defender.X + radius; x++)
            //    {
            //        for (int y = defender.Y - radius; y <= defender.Y + radius; y++)
            //        {
            //            int z = map.GetAverageZ(x, y);
            //            Effects.SendLocationEffect(new Point3D(x, y, z), map, randomHue, 25, 10, 33, blood.Hue);
            //        }
            //    }
            //} 
            ...
            {
                case SpecialAttacks.SilenceTarget:
                    {
                        defender.Freeze(TimeSpan.FromSeconds(10));
                        attacker.Emote("*Silence Target!*");
                        defender.FixedEffect(0x376A, 9, 32);
                        defender.PlaySound(0x204);
                        break;
                    }
                case SpecialAttacks.CriticalHit:
                    {
                        defender.Damage(weapon.MaxDamage * 3, attacker);
                        attacker.Emote("*Critical Hit!*");

                        int range = Utility.RandomMinMax(5, 7);
                        int zOffset = defender.Mounted ? 20 : 10;

                        Point3D src = defender.Location.Clone3D(0, 0, zOffset);
                        Point3D[] points = src.GetAllPointsInRange(defender.Map, 0, range);

                        Effects.PlaySound(defender.Location, defender.Map, 0x51E);

                        defender.FixedParticles(0x36BD, 20, 10, 5044, 137, 0, EffectLayer.Head);
                        ...
                        Timer.DelayCall(
                            ...
                            () =>
                            {
                                ...
                                foreach (Point3D trg in points)
                                {
                                    ...
                                    new MovingEffectInfo(src, trg.Clone3D(0, 0, 2), defender.Map, bodypartID).MovingImpact(
                                        info =>
                                        {
                                            Item bodypart;
                ...
                case SpecialAttacks.CriticalHit2:
                    {
                        defender.Damage(weapon.MaxDamage * 3, attacker);
                        attacker.Emote("*Critical Hit!*");
                ...
                case SpecialAttacks.LightningStrike:
                    {
                        attacker.Emote("*Lightning Strike!*");
                        Effects.SendBoltEffect(defender, true, 0, false);
                        defender.Damage(weapon.MaxDamage * 2, attacker);
                        break;
                    }
                case SpecialAttacks.BloodDrink:
                    {
                        attacker.FixedEffect(0x376A, 9, 32);
                        attacker.PlaySound(0x202);
                        attacker.Hits = attacker.HitsMax;
                        attacker.Emote("*Blood Drink!*");
                ...
                case SpecialAttacks.PlagueTouch:
                    {
                        attacker.Emote("*Plague Touch!*");

                        defender.PlaySound(0x43F);
        ...
        () =>
        {
            int i = 0;
            int place = 0;
            int[] PoisonGasEffectArray = { 9686 };

            foreach (Point3D trg in points)
            {
                i++;
                int poisonEffectID = Utility.RandomList(PoisonGasEffectArray);

                new MovingEffectInfo(src, trg.Clone3D(0, 0, 2), defender.Map, poisonEffectID).MovingImpact(
                    info =>
                    {
                        Item poisonEffect = new Item(poisonEffectID);
                        poisonEffect.MoveToWorld(info.Target.Location, info.Map);

                        Timer.DelayCall(TimeSpan.FromSeconds(0.2), () =>
                        {
                            if (!poisonEffect.Deleted)
                                poisonEffect.Delete();
                        });
                    });
            }
        ...
        {
            if (attacker == null || defender == null || weapon == null || m_SelectedAttack == null || defender.Player) return;
        ...
        private class CustomAttacksGump : Gump
        {
            ...
            public CustomAttacksGump(Mobile from, XmlCustomAttacks a) : base(0, 0)
            {
                ...
                AddLabel(15, 2, 55, "Exodus"); // Default "Attack"
                ...
                else
                {
                    ...
                    AddImage(15, 25, 0x66F); // default AddImage( 15, 25, 0x0a52 );
                }
