...
namespace Server.Misc
{
    public class GameTimeGoldRewardTimer : Timer
    {
        ...
        private static int GoldQuantity = 15000; // How much gold should we reward?

        private static int MinutesOnline = 60; // Every X minutes we give the reward.
        ...
        private static int MakeCheckAfterRewarded = 15000; // At what quantity should we start convert the gold to checks?
            ...
            foreach (NetState state in NetState.Instances)
            {
                ...
                if (m != null && m is PlayerMobile && m.AccessLevel >= RewardAccessLevel)
                {
                    if (DropOnBank && m.BankBox != null)
                    {
                        ...
                        m.SendMessage(33, "You have been rewarded for 1 hour of continued playtime!");
