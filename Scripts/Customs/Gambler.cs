        ...
        public class PokerGump : Gump
        {
            ...
            public void finishpokergame(Mobile from)
            {
                ...
                if (!isStrt && !isFlush)
                {
                    ...
                    {
                        if (match1 == 1) { temp = m_From.CardValue2(m_From.dealercards[0]); }
                        if (match2 == 1) { temp = m_From.CardValue2(m_From.dealercards[1]); }
                        if (match3 == 1) { temp = m_From.CardValue2(m_From.dealercards[2]); }
                        if (match4 == 1) { temp = m_From.CardValue2(m_From.dealercards[3]); }
                        if (temp >= 10)
                        {
