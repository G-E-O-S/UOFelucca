...
{
	public class InquisitionDeviantAbility : ExplosionDeviantAbility
	{
		public override string Name { get { return "Inquisition"; } }

		public override DeviationFlags Deviations { get { return DeviationFlags.Despair | DeviationFlags.Death | DeviationFlags.Decay; } }
		...
		public override TimeSpan Duration { get { return TimeSpan.FromSeconds(5); } } // default 10

		protected override BaseExplodeEffect CreateEffect(BaseDeviant deviant)
		{
			var range = (int)Math.Ceiling(deviant.RangePerception * (deviant.Hits / (double)deviant.HitsMax));

			return new SmokeExplodeEffect(deviant.Location, deviant.Map, Math.Max(5, range));
		}
		...
		protected override void OnLocked(BaseDeviant deviant)
		{
			...
			deviant.Yell("JUSTIFY YOUR EXISTENCE!");
		}
		...
		protected override void OnUnlocked(BaseDeviant deviant)
		{
			...
			deviant.Yell("YOUR INQUISITION IS NIGH!");
		}
		...
		protected override void OnAdded(State state)
		{
			...
			state.Deviant.Hits -= (int)Math.Floor(10.0 * Duration.TotalSeconds);
			state.Deviant.Mana -= (int)Math.Floor(30.0 * Duration.TotalSeconds);
		...
		protected override void OnRemoved(State state)
		{
			...
			state.Target.SendMessage(state.Deviant.SpeechHue, "[{0}]: The inquisition is over.", state.Deviant.RawName);
		}
