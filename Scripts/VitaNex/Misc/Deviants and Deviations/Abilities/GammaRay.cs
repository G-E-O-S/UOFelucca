...
{
	public class GammaRayDeviantAbility : DeviantAbility
	{
		...
		public override DeviationFlags Deviations { get { return DeviationFlags.Energy | DeviationFlags.Light | DeviationFlags.Tech; } }
		...
		protected override void OnInvoke(BaseDeviant deviant)
		{
			...
			for (int a = (cw ? 0 : 360), h = 0; (cw ? a <= 360 : a >= 0); a += (cw ? 1 : -1))
			{
				...
				{
					Deferred = false,
					Handler = e => HandleGammaRay(deviant, e)
				};
				...
				for (var i = 0; i < l.Length; i++)
				{
					...
					q.Add(
						new EffectInfo(p, deviant.Map, effect, hue, 10, 10, EffectRender.LightenMore)
						{
							QueueIndex = i
						});
		...
		private void HandleGammaRay(BaseDeviant deviant, EffectInfo e)
		{
