...
namespace Server.Misc
{
	public class SkillCheck
	{
		...
		static SkillCheck()
		{
			...
			_StatGainDelay = Config.Get("PlayerCaps.PlayerStatTimeDelay", TimeSpan.FromMinutes(5.0));
			_PetStatGainDelay = Config.Get("PlayerCaps.PetStatTimeDelay", TimeSpan.FromMinutes(1.0));

			_PlayerChanceToGainStats = Config.Get("PlayerCaps.PlayerChanceToGainStats", 15);
			_PetChanceToGainStats = Config.Get("PlayerCaps.PetChanceToGainStats", 15);
        ...
        private static double GetGainChance(Mobile from, Skill skill, double chance, bool success)
        {
            ...
            if (gc < 0.01)
                gc = 1.00; // default 0.01;

            // Pets get a 100% bonus
            if (from is BaseCreature && ((BaseCreature)from).Controlled)
                gc += gc * 2.50; // default 1.00
