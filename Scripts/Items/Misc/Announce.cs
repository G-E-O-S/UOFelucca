...
namespace Server
{
    public class Announce
    {
        ...
        private readonly static int m_LoginHue = 1161;//Login Message Hue
        private readonly static string m_LogoutMessage = "{0} has logged out.";//Logout Message
        private readonly static int m_LogoutHue = 1161;//Logout Message Hue		
        ...
        private static void World_Login(LoginEventArgs args)
        {
            ...
            {
                CommandHandlers.BroadcastMessage(AccessLevel.Player, m_LoginHue, String.Format(m_LoginMessage, args.Mobile.Name)); // default counselor
            }
            if (args.Mobile.AccessLevel >= AccessLevel.Administrator)
            {
