...
namespace Server.Items
{
    ...
    {
        public static readonly TimeSpan DefaultDuration = TimeSpan.FromDays(21.0);
        public static readonly TimeSpan ExpirationTime = TimeSpan.FromDays(21.0);
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                from.SendMessage(MessageHue, "You have already entered this raffle.");
            }
            else if (this.IsAtIPLimit(from))
            {
                from.SendMessage(MessageHue, "You may not enter this raffle.");
            }
            else
            {
                from.SendGump(new WarningGump(1150470, 0x7F00, String.Format("You are about to purchase a raffle ticket.  The ticket price is {1}.  Tickets are non-refundable and you can only purchase one ticket per account.  Do you wish to continue?", this.FormatLocation(), this.FormatPrice()), 0xFFFFFF, 420, 280, new WarningGumpCallback(Purchase_Callback), null)); // CONFIRM TICKET PURCHASE
            }
        }
        ...
        public void Purchase_Callback(Mobile from, bool okay, object state)
        {
            ...
            if (okay)
            {
                ...
                if (this.m_TicketPrice == 0 || (from.Backpack != null && from.Backpack.ConsumeTotal(typeof(Gold), this.m_TicketPrice)) || Mobiles.Banker.Withdraw(from, m_TicketPrice, true))
                {
                    ...
                    from.SendMessage(MessageHue, "You have successfully entered the raffle.");
                }
                else
                {
        ...
        public void CheckEnd()
        {
            ...
            if (this.m_Region != null && this.m_Entries.Count != 0)
            {
                ...
                if (this.m_Winner != null)
                {
                    ...
                    this.m_Winner.SendMessage(MessageHue, "Congratulations, {0}!  You have won the raffle!.", this.m_Winner.Name, this.FormatLocation());

                    if (this.m_Winner.AddToBackpack(this.m_Deed))
                    {
                        this.m_Winner.SendMessage(MessageHue, "The winnings has been placed in your backpack.");
                    }
                    else
                    {
                        this.m_Winner.BankBox.DropItem(this.m_Deed);
                        this.m_Winner.SendMessage(MessageHue, "As your backpack is full, the winnings has been placed in your bank box.");
                    }
                }
            }
