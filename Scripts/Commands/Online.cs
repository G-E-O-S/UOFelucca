...
namespace Server.Gumps
{
    public class OnlineGump : Gump
    {
        ...
        {
            CommandSystem.Register("Online", AccessLevel.Player, new CommandEventHandler(OnlineList_OnCommand));            
        }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous
                    {
                        if (this.m_Page > 0)
                            from.SendGump(new OnlineGump(from, this.m_Mobiles, this.m_Page - 1));
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < this.m_Mobiles.Count)
                        {
                            ...
                            if (m.Deleted)
                            {
                                ...
                                from.SendGump(new OnlineGump(from, this.m_Mobiles, this.m_Page));
                            }
        ...
        [Usage("OnlineList [filter]")]
        [Aliases("Online")]
        [Description("Lists all connected clients. Optionally filters results by name.")]
        private static void OnlineList_OnCommand(CommandEventArgs e)
        {
            e.Mobile.SendGump(new OnlineGump(e.Mobile, e.ArgString));
        }
