    ...
    public class TelekinesisSpell : MagerySpell
    {
        ...
        public void Target(Container item)
        {
            if (this.CheckSequence())
            {
                ...
                {
                    Effects.SendLocationParticles(EffectItem.Create(item.Location, item.Map, EffectItem.DefaultDuration), 0x376A, 9, 32, Caster.SpellHue != 0 ? Caster.SpellHue : 0, 0, 5022, 0);
